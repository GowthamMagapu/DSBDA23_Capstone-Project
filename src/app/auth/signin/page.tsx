...
const benefits = [
  'One workspace for every AI teammate',
    ...
    <main className="min-h-screen bg-black text-white">
      <header className="mx-auto flex w-full max-w-[1280px] items-center justify-between px-6 py-5 lg:px-8">
        ...
        <p className="text-sm text-white/55">
          New here?{' '}
          <Link href="/auth/signup" className="text-white underline decoration-white/30 underline-offset-4 transition-colors hover:text-white/70">
            Create an account
          </Link>
        </p>
      </header>
      ...
      <section className="mx-auto grid min-h-[calc(100vh-81px)] w-full max-w-[1280px] items-center gap-16 px-6 py-12 lg:grid-cols-[1fr_460px] lg:px-8 lg:py-16">
        ...
        <div className="rounded-[24px] border border-white/15 bg-[#111111] p-6 shadow-[0_30px_100px_rgba(0,0,0,0.5)] sm:p-8">
          ...
          <Link href="/" className="mt-6 flex items-center justify-center gap-2 text-sm text-white/45 transition-colors hover:text-white">
            Back to homepage <ArrowRight className="h-4 w-4 rotate-180" />
          </Link>
        </div>
      </section>
    </main>
